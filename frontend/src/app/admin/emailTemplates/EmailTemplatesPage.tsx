                    ...
                    {templates.map((t: EmailTemplate) => (
                        <div key={t._id} className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 shadow-sm shadow-black/20 flex flex-col gap-3">
                            <div className="flex justify-between items-start">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => handleEdit(t)} className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-zinc-300">
                                        <Edit2 className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleDelete(t._id)} className="p-2 bg-zinc-800 hover:bg-red-500/20 rounded-lg text-red-400 hover:text-red-300">
                                        <Trash2 className="w-4 h-4" />
                                    </button>
                                </div>
                            </div>
                            <div className="pt-3 border-t border-zinc-800 flex flex-col gap-1 text-sm">
                                <span className="text-zinc-500">Subject: <span className="text-zinc-300">{t.subject}</span></span>
